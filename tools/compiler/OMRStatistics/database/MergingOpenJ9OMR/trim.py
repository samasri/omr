# This script takes an SQL file (generated by the makefile) and makes sure to enforce any constraints on it. Depending on the filename, which is the same as the table name, the script will apply the right constraints)
...
if filename == 'Class.sql':
	...
	for row in file:
		...
		# Get info from row
		query = row[row.index('(') + 2 : row.rfind(')') - 1].split("','")
		id = query[0]
		namespace = query[1]
		clas = query[2]
		key = namespace + '-->' + clas
...
if filename == 'Hierarchy.sql':
	...
	for row in file:
		...
		if classID in classIDMapping:
			...
			id = int(query[0])
			classID = int(query[1])
		key = str(id) + '::' + str(classID)
		# Remove duplicates
		if key in keySet: 
			# No duplicates found, if any duplicates found a sameHierarchyMap should be created
			continue
		keySet[key] = 1
		write.write(row)
...
if filename == 'Polymorphism.sql':
	...
	for row in file:
		rc += 1
		if rc < 11: write.write(row)
		if row[:6] != 'INSERT': continue
		# Get info from row
		query = row[row.index('(') + 2 : row.rfind(')') - 1].split("','")
		HierarchyID = int(query[0])
		ChildClassID = int(query[1])
		ParentClassID = int(query[2])
		if ChildClassID in classIDMapping:
			row = row.replace("'" + str(ChildClassID) + "'", "'" + str(classIDMapping[ChildClassID]) + "'")
			# Reload query
			query = row[row.index('(') + 2 : row.rfind(')') - 1].split("','")
			HierarchyID = int(query[0])
			ChildClassID = int(query[1])
		...
		if ParentClassID in classIDMapping:
			row = row.replace("'" + str(ParentClassID) + "'", "'" + str(classIDMapping[ParentClassID]) + "'")
			# Reload query
			query = row[row.index('(') + 2 : row.rfind(')') - 1].split("','")
			HierarchyID = int(query[0])
			ChildClassID = int(query[1])
			ParentClassID = int(query[2])
		# Remove duplicates
		key = str(HierarchyID) + '::' + str(ChildClassID) + '::' + str(ParentClassID)
		if key in keySet: 
			continue
		keySet[key] = 1
		write.write(row)